...
SCRAPER_LOCATION=/var/www/court-listener/alert/scrapers/scrape_and_extract.py
...
case "${1:-''}" in
  ...
  'stop')
           ...
           kill -2 `ps -eo pid,command | grep 'scrape_and_extract.py -d' | grep -v 'grep' | awk -F' ' '{print $1}';`
           rm /tmp/scraper.pid
           echo "Done"
        ;;
  'restart')
