...
function checkDeps {
    ...
    deps=(aptitude antiword checkinstall g++ gcc git-core ipython libmysqlclient-dev libwpd-tools logrotate make mercurial mysql-client mysql-server poppler-utils python python-beautifulsoup python-chardet python-docutils python-mysqldb python-pip python-setuptools subversion tar wget)
    echo -e "\n########################"
