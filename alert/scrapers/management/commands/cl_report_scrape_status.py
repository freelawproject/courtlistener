...
def _num_weekdays(start_date, end_date=now()):
...
def calculate_counts():
    ...
    cts_last_day = Court.objects \
        .filter(docket__documents__date_filed__gt=last_day) \
        .annotate(count=Count('docket__documents__pk')) \
        .values('pk', 'count')
    cts_seven_days = Court.objects \
        .filter(docket__documents__date_filed__gt=seven_days_ago) \
        .annotate(count=Count('docket__documents__pk')) \
        .values('pk', 'count')
    ...
    # Reformat the results into dicts...
    cts_last_day = _make_query_dict(cts_last_day)
    cts_seven_days = _make_query_dict(cts_seven_days)
    cts_thirty_days = _make_query_dict(cts_thirty_days)
...
def generate_report():
    ...
    """
    averages, totals, critical_history = calculate_counts()
    errors, critical_today = tally_errors()
