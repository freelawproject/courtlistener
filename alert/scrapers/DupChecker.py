...
class DupChecker(dict):
    def __init__(self, court_id, full_crawl=False, dup_threshold=5):
    ...
    def _court_changed(self, court_id, hash):
        ...
        """
        url2Hash, created = urlToHash.objects.get_or_create(url=court_id)
    ...
    def abort_by_hash(self, hash):
        """Checks whether we should abort due to a hash of the site data being
        unchanged since the last time a court was scraped.
        ...
        self.url2Hash.
        """
        changed, self.url2Hash = self._court_changed(self.court_id, hash)
        if not self.full_crawl:
            if not changed:
                logger.info("Unchanged hash for: %s" % self.court_id)
                return True
            else:
                logger.info("Identified changed hash for: %s" % self.court_id)
                return False
        else:
            # If it's a full crawl, we don't care about the hash. We do not
        ...
        if exists:
            ...
            if not self.full_crawl:
                if already_scraped_next_date:
                    if 'state.mich' in self.court_id:
