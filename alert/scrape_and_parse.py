...
# result and verbosity should both be moved to global vars.
import settings
...
def getPDFContent(docs, result, verbosity):
...
def scrapeCourt(courtID, result, verbosity, daemonmode):
        ...
        for url in urls:
            ...
            while i > 0:
                ...
                logger.debug(time.strftime("%a, %d %b %Y %H:%M", time.localtime()) + 
                    ": Added " + ct.courtShortName + doc.caseNameShort)
                doc.save()
...
def parseCourt(courtID, result, verbosity):
