...
class PagerankTest(TestCase):
    ...
    def test_pagerank_calculation(self):
        ...
        """
        #ensure that there are no other docs in the database
        if not models.Document.objects.all().count() == 0:
            print("Please clear the documents in the database...")
            return
        if not models.Citation.objects.all().count() == 0:
            print("Please clear the citations in the database...")
            return

        #create 3 documents with their citations
        c1 = models.Citation(case_name="c1", federal_cite_one='1 Yeates 1 (test 1795)')
        c1.save(index=False)
        d1 = models.Document(
            date_filed=date(1795, 6, 9),
            court=self.court,
        ...
        c2 = models.Citation(case_name="c2")
        c2.save(index=False)
        d2 = models.Document(
            date_filed=date(1982, 6, 9),
            court=self.court,
        ...
        c3 = models.Citation(case_name="c3")
        c3.save(index=False)
        d3 = models.Document(
            date_filed=date(2000, 6, 9),
            court=self.court,
            citation=c3,
            plain_text="1 Yeates 1"
        )
        ...
        #create citing relation: 1 cites 2; 2 cites 3; 3 cites 1; 1 cites 3
        ...
        comm = cl_calculate_pagerank.Command()
        comm.handle()
        ...
        D2_ANSWER = 0.64443
        D3_ANSWER = 1.19219
        condition1 = abs(d1.pagerank-D1_ANSWER)/D1_ANSWER < 0.0001
        condition2 = abs(d2.pagerank-D2_ANSWER)/D2_ANSWER < 0.0001
        condition3 = abs(d3.pagerank-D3_ANSWER)/D3_ANSWER < 0.0001
        result = condition1*condition2*condition3
        self.assertEqual(
            result,
            1,
            msg="The pagerank calculation is wrong.\n"+
                "Your answer1: "+str(d1.pagerank)+"\tRIGHT ANSWER: "+str(D1_ANSWER)+"\n"+
                "Your answer2: "+str(d2.pagerank)+"\tRIGHT ANSWER: "+str(D2_ANSWER)+"\n"+
                "Your answer3: "+str(d3.pagerank)+"\tRIGHT ANSWER: "+str(D3_ANSWER)+"\n"
        )
