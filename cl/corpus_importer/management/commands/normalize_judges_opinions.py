...
from cl.people_db.lookup_utils import lookup_judge_by_last_name, \
    lookup_judges_by_last_name_list
from cl.search.models import Opinion, OpinionCluster


class Command(VerboseCommand):
    help = "Populate author field in opinions using author_str field and " \
    ...
    def normalize_panel_in_opinioncluster(self):
        ...
        for opinion_cluster in opinion_clusters_with_judges_str:
            ...
            court_id = opinion_cluster.docket.court.pk

            # Convert judges last name string to list
            last_name_list = opinion_cluster.judges.split(", ")

            # Search for judges as Person objects
            people = lookup_judges_by_last_name_list(last_name_list, court_id,
                                                     date_filed, False)

            if not people:
                logger.warning(
                    f"Can't find these people: {opinion_cluster.judges} in "
