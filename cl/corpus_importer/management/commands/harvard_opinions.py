...
def add_citations(cites: List[CitationType], cluster_id: int) -> None:
    ...
    for cite in cites:
        ...
        # We can only get one result due model constraints
        citation_obj = Citation.objects.filter(
        ...
        if citation_obj:
            if citation_obj.type != reporter_type:
                # Update reporter type if this changed in reporters-db
                citation_obj.type = reporter_type
                citation_obj.save()
        else:
            try:
