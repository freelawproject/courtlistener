...
def lookup_row(row):
    ...
    for args, kwargs in filter_tuples:
        ...
        if count == 1:
            logger.info(
                "Got one result. Bingo (args: %s, kwargs: %s)."
                % (args, kwargs)
            )
            return results[0]
        elif 5 > count > 1:
            logger.info(
                "Got %s results. Choosing closest to document date." % count
            )
            return results[0]
        else:
            logger.warn(
...
def update_csv_with_idb_lookups(options):
    ...
    """
    with open(options["input_file"], "r") as f, open(
        "/tmp/final-pull-annotated.csv", "wb"
    ) as o:
...
def download_dockets(options):
    ...
    with open(options["input_file"], "r") as f:
        ...
        throttle = CeleryThrottle(
            queue_name=q, min_items=options["queue_length"]
        )
        ...
        try:
            if any(
                [
                    ...
                    # as the second word
                    words[0] in prefix_words and words[1] in core_words,
                    # 'memorandum' or 'report' as the 1st word followed
                    # by a core word as the second or third word
                    words[0] in pre_prefix_words
                    and (words[1] in core_words or words[2] in core_words),
                    # A prefix word as the first word followed by 'memorandum' or
                    # 'report' as the second word followed by a core word as the
                    # 3rd or 4th word
                    words[0] in prefix_words
                    and words[1] in pre_prefix_words
                    and (words[2] in core_words or words[3] in core_words),
                ]
            ):
                good_des.append(de)
        except IndexError:
...
def download_documents(options):
    ...
    for i, row in enumerate(reader):
        ...
        docket_number = (
            row["cl_d_docket_number"]
            or row["cl_d_docket_number (student)"]
            or None
        )
                ...
                if options["task"] == "add_extra_tags":
                    ...
                    chain(
                        get_pacer_doc_by_rd.s(
                            rd.pk, session.cookies, tag=TAG_NAME
                        ).set(queue=q),
                        extract_recap_pdf.si(rd.pk).set(queue=q),
                        add_items_to_solr.si(
                            [rd.pk], "search.RECAPDocument"
                        ).set(queue=q),
                    ).apply_async()
    f.close()
...
class Command(VerboseCommand, CommandUtils):
    ...
    def handle(self, *args, **options):
        ...
        if options["task"] == "lookup_in_idb":
            ...
            options["task"] == "download_dockets"
            or options["task"] == "download_student_dockets"
        ):
            download_dockets(options)
        elif (
            options["task"] == "download_documents"
            or options["task"] == "add_extra_tags"
        ):
