...
--
-- The Python step backfills token_checksum = sha256(token) for every existing
-- row. On a replica those UPDATEs arrive through replication; wait for it to
-- catch up (no rows with token_checksum IS NULL) before running SET NOT NULL
-- below, or it will fail.
--
