# Generated by Django 5.1.8 on 2025-09-11 18:33

import pgtrigger.compiler
import pgtrigger.migrations
from django.db import migrations, models

def populate_docket_number_raw(apps, schema_editor):
    """
        Populates the new docket_number_raw field for all Docket records in batches.

        Batching is used to avoid locking the entire table and to reduce memory usage,
        which is important for large datasets. This approach ensures the migration
        remains efficient and minimizes database contention.
    """
    Docket = apps.get_model('search', 'Docket')
    batch_size = 10000
    last_pk = None
    qs = Docket.objects.order_by('pk')
    while True:
        batch = qs
        ...
        if not items:
            break
        Docket.objects.filter(pk__in=[obj.pk for obj in items]).update(
            docket_number_raw=models.F('docket_number')
        )
        last_pk = items[-1].pk
    ...
    operations = [
        ...
        migrations.AddField(
            ...
            field=models.CharField(
                blank=True,
                help_text='This field would store all of the docket number values exactly as we’ve extracted them from our various sources.',
                null=True,
            ),
        ),
        migrations.AddField(
