...
Process is as follows:
    For each row in the CSV:
        ...
        Search for the case number using a query such as @west_cite 107 676 @court scotus
...
def gen_diff_ratio(case_name_left, case_name_right):
    ...
    diff = difflib.SequenceMatcher(
        None, case_name_left.strip(), case_name_right.strip()
    ).ratio()
        ...
        if results.count() == 0:
            ...
            if diff >= HIGH_THRESHOLD:
                ...
                if not simulate:
                    ...
                    results[0].date_filed = datetime.date(
            ...
            if max_ratio >= THRESHOLD:
                ...
                if not simulate:
                    splitDate = csv_date_published.split("-")
                    results[i].date_filed = datetime.date(
                        int(splitDate[0]), int(splitDate[1]), int(splitDate[2])
                    )
                    results[i].save()
