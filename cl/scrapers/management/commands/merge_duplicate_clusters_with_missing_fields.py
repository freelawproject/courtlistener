from cl.search.models import Opinion, OpinionCluster, PRECEDENTIAL_STATUS, Citation
...
def extract_opinion_metadata(text):
    ...
    case_name_match = re.match(r'\s*(.*?)\s*\r?\n\d{4}\s+[A-Z]{1,3}\s+\d+',text)
    data['case_name'] = case_name_match.group(1).strip() if case_name_match else None

    #Find docket number or case number in the text
    docket_match = re.search(r'Case Number:\s*(\d+)', text)
    data['docket_number'] = docket_match.group(1).strip() if docket_match else None
    ...
    attorneys = re.findall(r'(\*?\d*\s*[A-Z][A-Za-z\s\.]+,.*?,\s*for (?:Plaintiff|Defendant)/Appellee)', text)
...
def ready_to_merge(keeper, clust):
   ...
   #Checking if clusters opinions have conflicts
    for other_cluster_opinion in other_cluster_opinions:
        for keeper_opinion in keeper_opinions:
            ...
            if other_cluster_opinion.type == keeper_opinion.type:
                ...
                if other_cluster_opinion.plain_text != keeper_opinion.plain_text:
                    return 0
    ...
    fields_to_check = {
        ...
        "scdb_votes_majority":(
            keeper.scdb_votes_majority, clust.scdb_votes_majority),
        "scdb_votes_minority":(
            keeper.scdb_votes_minority, clust.scdb_votes_minority),
        "scdb_decision_direction":(
            keeper.scdb_decision_direction, clust.scdb_decision_direction),
    ...
    for other_cluster_citation in other_cluster_citations:
        for keeper_citation in keeper_citations:
            for field in ["volume", "reporter", "page"]:
                ...
                other_cluster_citation_val = getattr(other_cluster_citation, field)

                if (keeper_citation_val not in None) and (other_cluster_citation_val not in None) and (keeper_citation_val != other_cluster_citation_val):
                    return 0
    ...
    for field_name, (keeper_val, clust_val) in fields_to_check.items():
        ...
        if field_name in ["syllabus", "summary", "arguments", "headnotes", "headmatter"]:
            if keeper_val and clust_val and not text_is_similar(keeper_val, clust_val):
                return 0
        else:
    ...
    def handle(self, *args, **kwargs):
        ...
        with transaction.atomic():
            ...
            for dup in duplicates:
                clusters = list(OpinionCluster.objects.filter(case_name=dup["case_name"]).order_by("id"))
                ...
                for i in range(len(clusters)):
                    ...
                    for j in range(i + 1, len(clusters)):
                        ...
                        if keeper.docket.court.fjc_court_id != other_cluster.docket.court.fjc_court_id:
                            continue

                        if keeper.docket.docket_number != other_cluster.docket.docket_number:
                            continue
                        #---------------------------------------------#
                        ...
                        other_cluster_opinions = list(Opinion.objects.filter(cluster=other_cluster))
                        ...
                        for judge in other_cluster.non_participating_judges.all():
                            keeper.non_participating_judges.add(judge)

                        #Fields that need to be merged
                        fields_to_transfer = {
                            "summary":(keeper.summary, other_cluster.summary),
                            "syllabus":(keeper.syllabus, other_cluster.syllabus),
                            "arguments":(keeper.arguments, other_cluster.arguments),
                            "headnotes":(keeper.headnotes, other_cluster.headnotes),
                            "headmatter":(keeper.headmatter, other_cluster.headmatter),
                            "citation_count":(keeper.citation_count, other_cluster.citation_count),
                            "scdb_votes_majority":(keeper.scdb_votes_majority, other_cluster.scdb_votes_majority),
                            "scdb_votes_minority":(keeper.scdb_votes_minority, other_cluster.scdb_votes_minority),
                            "scdb_decision_direction":(keeper.scdb_decision_direction, other_cluster.scdb_decision_direction),
                            "scdb_id":(keeper.scdb_id, other_cluster.scdb_id),
                            "slug":(keeper.slug, other_cluster.slug),
                            "judges":(keeper.judges, other_cluster.judges),
                            "attorneys":(keeper.attorneys, other_cluster.attorneys),
                            "nature_of_suit":(keeper.nature_of_suit, other_cluster.nature_of_suit),
                            "posture":(keeper.posture, other_cluster.posture),
                            "disposition":(keeper.disposition, other_cluster.disposition),
                            "history":(keeper.history, other_cluster.history),
                            "other_dates":(keeper.other_dates, other_cluster.other_dates),
                            "cross_reference":(keeper.cross_reference, other_cluster.cross_reference),
                            "correction":(keeper.correction, other_cluster.correction),
                        }

                        #Transfering fields to the keeper
                        for field_name, (kep, other_cluster_value) in fields_to_transfer.items():
                            ...
                            if not keeper_val and other_cluster_value:
                                setattr(keeper, field_name, other_cluster_value)
                ...
                logger.info(f"Merged validated duplicates for case: {dup['case_name']}")
