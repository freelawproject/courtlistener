    ...
    def xml_response(self):
        response_portions = [
            ...
            E.result(
                {
                    "name": "response",
                    "numFound": str(len(self.mock_docs)),
...
class MockConnection(object):
    ...
    def request(self, uri, method="GET", body=None, headers=None):
        ...
        if (
            method == "GET"
            and u.path.endswith("/admin/file/")
            and params.get("file") == ["schema.xml"]
        ):
...
class PaginationMockConnection(MockConnection):
    def _handle_request(self, uri_obj, params, method, body, headers):
        if method == "GET" and uri_obj.path.endswith("/select/"):
            ...
            return (
                self.MockStatus(200),
                MockResponse(start, rows).xml_response(),
            )


conn = SolrInterface(
    "http://test.example.com/", http_connection=PaginationMockConnection()
)
...
def check_slice_pagination(p_args, a, s):
    assert [d["int_field"] for d in conn.query("*").paginate(*p_args)[s]] == a[
        s
    ]
...
mlt_query_tests = (
    ...
    (
        ("Content with space", None, None),
        (
            {"stream.body": ["Content with space"], "mlt.fl": ["text_field"]},
            "GET",
            "",
        ),
        None,
    ...
    (
        (None, None, "http://source.example.com"),
        (
            {
                "stream.url": ["http://source.example.com"],
                "mlt.fl": ["text_field"],
            },
            "GET",
            "",
    ...
    (
        ("long " * 1024 + "content", None, None),
        ({"mlt.fl": ["text_field"]}, "POST", "long " * 1024 + "content"),
        None,
    ),
    ...
    (
        ("Cont\xe9nt", "iso-8859-1", None),
        (
            {"stream.body": ["Cont\xc3\xa9nt"], "mlt.fl": ["text_field"]},
            "GET",
            "",
        ),
        None,
...
def check_mlt_query(i, o, E):
    ...
    d = {}
    conn = SolrInterface(
        "http://test.example.com/", http_connection=MLTMockConnection(d)
    )
    if E is None:
        conn.mlt_query(
            content=content, content_charset=content_charset, url=url
        ).execute()
