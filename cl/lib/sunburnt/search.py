...
class LuceneQuery(object):
    ...
    def __len__(self):
        ...
        return sum(
            [
                sum(len(v) for v in self.terms.values()),
    ...
    def add(self, args, kwargs):
        ...
        for k, v in kwargs.items():
            ...
            if not field:
                ...
                raise SolrError(
                    "Can't query on non-indexed field '%s'" % field_name
                )
            if rel == "eq":
    ...
    def add_exact(self, field_name, values, term_or_phrase):
        ...
        if field_name and field_name != "*":
            ...
            if len(values) == 1 and values[0] == "*":
                ...
                raise SolrError(
                    "If field_name is '*', then only '*' is permitted as the query"
                )
        insts = [field.instance_from_user_data(value) for value in values]
        for inst in insts:
            if isinstance(field, SolrUnicodeField):
                this_term_or_phrase = term_or_phrase or self.term_or_phrase(
                    inst.value
                )
            else:
    ...
    def add_range(self, field_name, rel, value):
        ...
        if rel in ("range", "rangeexc"):
            ...
            insts = tuple(
                sorted(field.instance_from_user_data(v) for v in value)
            )
        elif rel == "any":
        ...
        if constructor is not dict:
            ...
            if result.highlighting:
                for d in result.result.docs:
                    ...
                    unique_key = self.schema.fields[
                        self.schema.unique_key
                    ].to_solr(d[self.schema.unique_key])
                    if (
                        "solr_highlights" not in d
                        and unique_key in result.highlighting
                    ):
        ...
        for field_name, field_opts in self.fields.items():
            if not field_name:
                ...
                for field_opt, v in field_opts.items():
                    opts[
                        "f.%s.%s.%s"
                        % (field_name, self.option_name, field_opt)
                    ] = v
        return opts
...
class FacetOptions(Options):
    ...
    opts = {
        ...
        and int(x)
        or self.invalid_value(),
        "mincount": lambda self, x: int(x) >= 0
        and int(x)
        or self.invalid_value(),
        "missing": bool,
        "method": ["enum", "fc"],
...
class HighlightOptions(Options):
    ...
    opts = {
        ...
        "alternateField": lambda self, x: x
        if x in self.schema.fields
