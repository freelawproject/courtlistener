...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        pgtrigger.migrations.AddTrigger(
            model_name='apithrottle',
            trigger=pgtrigger.compiler.Trigger(name='update_update', sql=pgtrigger.compiler.UpsertTriggerSql(condition='WHEN (OLD."blocked" IS DISTINCT FROM (NEW."blocked") OR OLD."id" IS DISTINCT FROM (NEW."id") OR OLD."notes" IS DISTINCT FROM (NEW."notes") OR OLD."rate" IS DISTINCT FROM (NEW."rate") OR OLD."throttle_type" IS DISTINCT FROM (NEW."throttle_type") OR OLD."user_id" IS DISTINCT FROM (NEW."user_id"))', func='INSERT INTO "api_apithrottlehistoryevent" ("blocked", "date_created", "date_modified", "id", "notes", "pgh_context_id", "pgh_created_at", "pgh_label", "pgh_obj_id", "rate", "throttle_type", "user_id") VALUES (OLD."blocked", OLD."date_created", OLD."date_modified", OLD."id", OLD."notes", _pgh_attach_context(), NOW(), \'update\', OLD."id", OLD."rate", OLD."throttle_type", OLD."user_id"); RETURN NULL;', hash='27571d239782ab7adeed184afb15ed50324afd6c', operation='UPDATE', pgid='pgtrigger_update_update_9846f', table='api_apithrottle', when='AFTER')),
        ),
        pgtrigger.migrations.AddTrigger(
            model_name='apithrottle',
            trigger=pgtrigger.compiler.Trigger(name='delete_delete', sql=pgtrigger.compiler.UpsertTriggerSql(func='INSERT INTO "api_apithrottlehistoryevent" ("blocked", "date_created", "date_modified", "id", "notes", "pgh_context_id", "pgh_created_at", "pgh_label", "pgh_obj_id", "rate", "throttle_type", "user_id") VALUES (OLD."blocked", OLD."date_created", OLD."date_modified", OLD."id", OLD."notes", _pgh_attach_context(), NOW(), \'delete\', OLD."id", OLD."rate", OLD."throttle_type", OLD."user_id"); RETURN NULL;', hash='fbcdd38c680b5e6c308ed26a23b166c7d5d33a78', operation='DELETE', pgid='pgtrigger_delete_delete_3cec8', table='api_apithrottle', when='AFTER')),
        ),
    ]
