...
from cl.audio.api_views import TranscriptViewSet
...
class TranscriptAPITest(TestCase):
    def setUp(self):
        ...
        self.audio = AudioFactory(stt_transcript="This is a test transcript.")
        self.permission = Permission.objects.get(
            codename="can_view_transcript"
        )

    def test_transcript_access_without_permission(self):
        view = TranscriptViewSet.as_view({"get": "retrieve"})
        request = self.factory.get(
            f"/api/rest/v3/audio/{self.audio.pk}/transcripts/{self.audio.pk}/"
        )
        force_authenticate(request, user=self.user)
        response = view(request, audio_pk=self.audio.pk, pk=self.audio.pk)
        self.assertEqual(response.status_code, 403)

    def test_transcript_access_with_permission(self):
        ...
        self.assertEqual(
            response.data["stt_transcript"], "This is a test transcript."
        )
