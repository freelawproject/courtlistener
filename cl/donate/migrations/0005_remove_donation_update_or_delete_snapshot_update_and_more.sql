BEGIN;
--
-- Remove trigger update_or_delete_snapshot_update from model donation
--
DROP TRIGGER IF EXISTS pgtrigger_update_or_delete_snapshot_update_a4f64 ON "donate_donation";
--
-- Remove trigger update_or_delete_snapshot_update from model monthlydonation
--
            ...
            RETURNS TRIGGER AS $$
                ...
                BEGIN
                    IF ("public"._pgtrigger_should_ignore(TG_NAME) IS TRUE) THEN
                        ...
                        END IF;
                    END IF;
                    INSERT INTO "donate_monthlydonationevent" ("date_created", "date_modified", "donor_id", "enabled", "failure_count", "id", "monthly_donation_amount", "monthly_donation_day", "payment_provider", "pgh_context_id", "pgh_created_at", "pgh_label", "pgh_obj_id", "stripe_customer_id") VALUES (OLD."date_created", OLD."date_modified", OLD."donor_id", OLD."enabled", OLD."failure_count", OLD."id", OLD."monthly_donation_amount", OLD."monthly_donation_day", OLD."payment_provider", _pgh_attach_context(), NOW(), 'update_or_delete_snapshot', OLD."id", OLD."stripe_customer_id"); RETURN NULL;
                END;
            $$ LANGUAGE plpgsql;

            DROP TRIGGER IF EXISTS pgtrigger_update_or_delete_snapshot_update_e645e ON "donate_monthlydonation";
            CREATE  TRIGGER pgtrigger_update_or_delete_snapshot_update_e645e
                AFTER UPDATE ON "donate_monthlydonation"
                
                
                FOR EACH ROW WHEN (OLD."id" IS DISTINCT FROM (NEW."id") OR OLD."date_created" IS DISTINCT FROM (NEW."date_created") OR OLD."donor_id" IS DISTINCT FROM (NEW."donor_id") OR OLD."enabled" IS DISTINCT FROM (NEW."enabled") OR OLD."payment_provider" IS DISTINCT FROM (NEW."payment_provider") OR OLD."monthly_donation_amount" IS DISTINCT FROM (NEW."monthly_donation_amount") OR OLD."monthly_donation_day" IS DISTINCT FROM (NEW."monthly_donation_day") OR OLD."stripe_customer_id" IS DISTINCT FROM (NEW."stripe_customer_id") OR OLD."failure_count" IS DISTINCT FROM (NEW."failure_count"))
                EXECUTE PROCEDURE pgtrigger_update_or_delete_snapshot_update_e645e();

            COMMENT ON TRIGGER pgtrigger_update_or_delete_snapshot_update_e645e ON "donate_monthlydonation" IS '8c2b5b9bee7d92d36aede241ceb08310cc28c951';
