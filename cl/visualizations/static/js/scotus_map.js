...
function drawGraph(target, opinions, chartType, axisType, height, maxDoS, mode, galleryId) {
  var workingJSON = JSON.parse(JSON.stringify(opinions)), // make a modifiable copy to retain original
    ...
    spaethLabels = ['L5-4', 'L6-3', 'L7-2', 'L8-1', 'N9-0', 'C8-1', 'C7-2', 'C6-3', 'C5-4', 'Unk'],
    index = 0,
    // chart parts
  ...
  if (!$(target).length) {
    // if the target isn't on the page stop now
    return [];
  }
  ...
  if (chartMode === 'dos') {
    ...
    if (ddc[3] === 0 && ddc[4] === 0) {
      // there are no unknown or unspecifiable
  ...
  if (xAxisMode === 'cat') {
    xGrid = new Plottable.Scales.Linear().domain([0, caseCount]).tickGenerator(function () {
  ...
  if (chartMode === 'dos') {
    yGrid.domain([0, 100]).tickGenerator(function () {
      return [0, 100];
    }); // handle dos and non dos
  } else if (ddc[3] > 0 || ddc[4] > 0) {
    // there is an 'Unk' category
    yGrid.domain([0, 11]);
  } else {
    yGrid.domain([0, 10]);
  }
...
*/
      ...
      workingJSON.forEach(function (cluster) {
        ...
        for (item in cluster.sub_opinions[0].opinions_cited) {
          if (cluster.sub_opinions[0].opinions_cited.hasOwnProperty(item)) {
            if (
              !workingJSON[JSONIndex[item].num].hasOwnProperty('visited') &&
  ...
  if (galleryId === '') {
    ...
    // gallery spaeth and genealogy mode
  ...
  if (galleryId === '') {
    ...
    caseHoverGroup = cases.foreground().append('g').attr('transform', 'translate(0,0)').style('visibility', 'hidden');
    caseHoverGroup.append('circle').attr({
      stroke: 'black',
      fill: 'none',
      r: 15,
      cx: 0,
    ...
    caseHover.onPointerMove(function (p) {
      ...
      if (datum !== null) {
        caseHoverText.text(datum.case_name ? datum.case_name : datum.case_name_short).style('visibility', 'visible');
        caseHoverGroup
        ...
        caseHoverText.style('visibility', 'hidden').text(defaultCaseHoverText);
...
function citationTable(target, data, columns) {
  ...
  thead
    ...
    .on('click', function (d) {
      ...
      table.selectAll('thead th').html(function (c) {
        var label = c.l;
...
function degreeOfDissent(cases) {
  ...
  return { d: d3.sum(dods) / dods.length, c: dods.length, l: cases.length };
}
...
function attachDoDInfo(target, dissent) {
  d3.select(target).text(dissent.d.toFixed(2) + ' for ' + dissent.c + ' cases of ' + dissent.l);
}
...
function casesMetadata(data) {
  ...
  if (data.length) {
    ...
    meter.append('text').attr('x', 10).attr('y', 9).text('0');
    meter.append('text').attr('x', 109).attr('y', 9).text('1');
