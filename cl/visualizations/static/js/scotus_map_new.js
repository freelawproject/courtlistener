$(document).ready(function () {
  var dateFiledQ =
    '((dateFiled:[* TO ' + last_year + 'Z] AND scdb_id:["" TO *]) OR (dateFiled:[' + last_year + 'Z TO *]))';
  var cache = {};
  ...
  var getParamsForQuery = function (query) {
    ...
    if ($.trim(query).length > 0) {
      ...
      params.case_name = '(%QUERY) OR (%QUERY*)'.replace(/%QUERY/g, $.trim(query));
    }
  ...
  $('#starting-cluster-typeahead, #ending-cluster-typeahead-search').typeahead(optionsMinSet, resultOptions);
  $('#ending-cluster-typeahead-citing, #ending-cluster-typeahead-authorities').typeahead(optionsNoMin, resultOptions);

  $('#starting-cluster-typeahead').on('typeahead:select', function (ev, suggestion) {
    updateCache(suggestion, function (suggestion) {
      $('.authority-count').text('(' + cache[suggestion.cluster_id].authority_count + ')');
      $('.citing-count').text('(' + cache[suggestion.cluster_id].citing_count + ')');
  ...
  $('.ending-typeahead').on('typeahead:select', function (ev, suggestion) {
