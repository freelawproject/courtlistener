...
def assign_authors_to_opinions(jurisdictions=None, testing=False):
    clusters = (
        OpinionCluster.objects.exclude(judges="")
    ...
    for cluster in clusters:
        ...
        candidates = get_candidate_judges(
            judge_str, cluster.docket.court_id, cluster.date_filed
        )
        if len(candidates) < 1:
            # No DB matches
            print("  No match.")
...
def assign_authors_to_oral_arguments(testing=False):
    ...
    for af in afs:
        ...
        candidates = get_candidate_judges(
            judge_str, af.docket.court_id, af.docket.date_argued
        )
        for candidate in candidates:
            if not testing:
