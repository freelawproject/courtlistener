...
for courtid, (state,level) in courtid2statelevel.items():  
    ...
    for year in range(1989,2017):
        ...
        judgelist = urllib.request.urlopen(url)    
        data = json.loads(judgelist.read().decode())
...
for i, row in judges.iterrows():
    ...
    judges = candidate_eid_lists[courtid]
    
    # check last name
    matches = [j for j in judges if lastname.lower() in j[1].lower()]
