#!/Users/pinguin/courtlistener/venv/bin/python
...
CATEGORIES = {
    ...
    sre_parse.CATEGORY_WORD: [
        unichr(x) for x in range(256) if match("\w", unichr(x), U)
    ],
        ...
        if i[0] == sre_parse.IN:
            ...
            if i[1][0] == i[1][1]:
                ...
                if i[1][0] == 0 and i[1][1] - i[1][0] == sre_parse.MAXREPEAT:
                    ...
                    i[1][0] == 1
                    and i[1][1] - i[1][0] == sre_parse.MAXREPEAT - 1
                ):
                    range_str = "+"
                else:
...
def __main__():
    ...
    if args["count"]:
        args["output"].write(
            "%d%s"
            % (count(args["regex"], limit=args["limit"]), args["delimiter"])
        )
        exit(0)
    if args["random"]:
        args["output"].write(
            "%s%s"
            % (getone(args["regex"], limit=args["limit"]), args["delimiter"])
        )
        exit(0)
    if args["simplify"]:
        args["output"].write(
